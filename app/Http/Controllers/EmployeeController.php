	...
	*/
	public  function empHistory(Request $request) {
		...
		foreach ($empdetails as $key => $value) {
			...
			$expdetails = Common::getYrMonCountBtwnDates($value->start_date,$value->end_date);
			$empdt = Common::fnGetEmployeeInfo($value->emp_id);
			if (isset($empdt[0]->FirstName) &&  isset($empdt[0]->LastName)) {
				$empdetailsdet[$i]['FirstName'] = $empdt[0]->FirstName;
				$empdetailsdet[$i]['LastName'] = $empdt[0]->LastName;
	    	}else{
